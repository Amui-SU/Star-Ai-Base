...
def test_frontend_provider_presets_are_shared():
    ...
    api_account_identity = (
        project_root
        / "frontend"
        / "components"
        / "api-accounts"
        / "ApiAccountIdentitySection.tsx"
    ).read_text(encoding="utf-8")
    api_account_workspace = (
        project_root
        / "frontend"
        / "components"
        / "api-accounts"
        / "useApiAccountWorkspace.ts"
    ).read_text(encoding="utf-8")
    ...
    assert "@/lib/providers" in api_account_identity
    assert "@/lib/providers" in api_account_workspace
    assert "const PROVIDERS" not in api_account_identity
    assert "const PROVIDERS" not in api_account_workspace
