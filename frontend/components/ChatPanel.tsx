  ...
  return (
    <div className="code-block-wrap">
      <div className="code-block-toolbar">
        ...
        <button type="button" className="code-copy-btn" onClick={() => void handleCopy()}>
          {copied ? "已复制" : "复制"}
        </button>
      </div>
      <pre>
...
export default function ChatPanel({ statsKey, sessionId, folderIds, sidebarOpen = true }: Props) {
  ...
  const builtInProviders: Array<{ provider: string; label: string; model: string }> = [
    ...
    { provider: "siliconflow", label: "SiliconFlow", model: "Qwen/Qwen2.5-7B-Instruct" },
    { provider: "zhipu", label: "智谱 GLM", model: "glm-4-flash" },
  ];

  const openProviderConfig = (provider: { provider: string; label: string; model: string; base_url?: string }) => {
    ...
    try {
      ...
      while (!done) {
        ...
        if (value) {
          ...
          if (chunk) {
            buffer += chunk;
            const markerIndexes = [buffer.indexOf(thinkingMarker), buffer.indexOf(sourcesMarker)].filter((idx) => idx >= 0);
            const firstMarkerIndex = markerIndexes.length > 0 ? Math.min(...markerIndexes) : -1;
            const visibleText = firstMarkerIndex >= 0 ? buffer.slice(0, firstMarkerIndex) : buffer;
            setMessages((prev) =>
              prev.map((m) =>
      ...
      const extractMarkerJson = (fullText: string, marker: string, nextMarker?: string) => {
      ...
      const thinkingJson = extractMarkerJson(buffer, thinkingMarker, sourcesMarker);
      const sourcesJson = extractMarkerJson(buffer, sourcesMarker);
      const contentEndIndexes = [buffer.indexOf(thinkingMarker), buffer.indexOf(sourcesMarker)].filter((idx) => idx >= 0);
      const contentEnd = contentEndIndexes.length > 0 ? Math.min(...contentEndIndexes) : buffer.length;
      ...
      const finalAnswer = parsedThinking
        ? (finalContent.trim() || (finalThinking ? "（已生成思考过程，展开查看）" : ""))
        : extracted.answer;

      setMessages((prev) =>
        prev.map((m) =>
          m.id === assistantId
            ? { ...m, content: finalAnswer, thinking: finalThinking || undefined, sources: parsedSources }
            : m
        )
      );
    } catch (err) {
      ...
      try {
        const res = await chatApi.ask(q, sessionId, folderIds, smartSearchEnabled, deepThinkEnabled);
        ...
        const finalAnswer = res.thinking
          ? ((res.answer || "").trim() || (finalThinking ? "（已生成思考过程，展开查看）" : ""))
          : extracted.answer;
        setMessages((prev) =>
          prev.map((m) =>
            m.id === assistantId ? { ...m, content: finalAnswer, thinking: finalThinking || undefined, sources: res.sources } : m
          )
        );
      } catch (err) {
        setMessages((prev) =>
  ...
  const extractThinkingFromContent = (rawText: string): { thinking: string; answer: string } => {
  ...
  const handleComposerChange = (value: string, target?: HTMLTextAreaElement | null) => {
    setInput(value);
  ...
  const handleCopyMessage = async (messageId: string, content: string) => {
    try {
      ...
      window.setTimeout(() => setCopiedMessageId((prev) => (prev === messageId ? null : prev)), 1500);
    } catch {
      setCopiedMessageId(null);
    }
  };

  const handleReaction = (messageId: string, reaction: Exclude<Reaction, null>) => {
    setReactionMap((prev) => ({
      ...prev,
  ...
  const handleSubmitEditedQuestion = async (messageId: string) => {
    ...
    setMessages((prev) => {
      ...
      return [...prev.slice(0, idx), editedUserMessage, { id: assistantId, role: "assistant", content: "", sources: [] }];
    });
  ...
  const remoteProviderMap = new Map(remoteProviders.map((p) => [p.provider, p]));
  const providersForMenu = [
    ...
    ...remoteProviders.filter((p) => !builtInProviders.some((b) => b.provider === p.provider)),
  ];
  const currentProvider = llmConfig?.current_provider ?? providersForMenu[0]?.provider;
  const activeProvider = providersForMenu.find((p) => p.provider === currentProvider);
    ...
    <div className="panel-inner">
      ...
      <div
        className={`fixed top-4 z-40 w-[calc(100vw-160px)] max-w-[960px] flex items-center justify-between ${
          sidebarOpen ? "left-1/2 -translate-x-1/2" : "left-[calc(50%-220px)] -translate-x-1/2"
        }`}
      >
          ...
          <div className="inline-flex items-center gap-0 rounded-full border border-(--border) bg-(--paper-2) p-0.5 shadow-sm">
            ...
            <div className="relative" ref={modelMenuRef}>
              <button
                ...
                title={activeProvider ? `${activeProvider.label} · ${activeProvider.model}` : "模型选择"}
                aria-label="模型选择"
              >
                {activeProvider && (
                  <>
                    <span className="inline-flex items-center justify-center w-3.5 h-3.5 rounded-sm">
                      <img
                        src={providerLogoMap[activeProvider.provider] || "/logos/qwen-icon.png"}
                        alt={`${activeProvider.label} logo`}
                        className="w-3 h-3 rounded-sm object-contain"
                      />
                    </span>
                    <span className="text-[10px] text-(--ink-soft) leading-none max-w-[96px] truncate">{activeProvider.label}</span>
                ...
                <div className="absolute top-[34px] left-1/2 -translate-x-1/2 z-20 w-[198px] max-w-[calc(100vw-32px)] rounded-xl border border-(--border) bg-(--paper-2) shadow-xl p-2 flex flex-col gap-2.5 backdrop-blur-sm">
                  {providersForMenu.map((p) => (
                    <button
                      ...
                      title={p.enabled ? `${p.label} · ${p.model}` : `${p.label}（未配置）`}
                    >
                      <span className="inline-flex min-w-0 flex-1 items-center gap-1.5 translate-x-4">
                        <span className="inline-flex items-center justify-center w-5 h-5 rounded-md bg-(--paper)">
                          <img
                            src={providerLogoMap[p.provider] || "/logos/qwen-icon.png"}
                            alt={`${p.label} logo`}
                            className="w-3 h-3 rounded-sm object-contain"
                          />
                        </span>
                        <span className="min-w-0">
                          <span className="block text-[10px] font-semibold text-(--ink-soft) leading-tight truncate">{p.label}</span>
                          <span className="block text-[9px] text-(--muted) leading-tight truncate mt-1">{p.model}</span>
                        </span>
                      </span>
                      <span className="inline-flex shrink-0 items-center gap-1 -translate-x-4">
                        ...
                        {p.provider === currentProvider && p.enabled && (
                          <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-[rgba(95,163,255,0.18)] text-(--accent-strong) text-[10px]">✓</span>
                        )}
                      </span>
                    </button>
                  ))}
          ...
          {messages.length === 0 ? (
            <div className="empty-state">
              <div>
                ...
                <p className="text-sm text-(--muted) mt-3">把收藏夹变成可提问的知识库</p>
              </div>
              ...
              {messages.map((m, idx) => (
                <div key={m.id} className={`message ${m.role}`}>
                  <div className="message-main">
                    <div className={`message-bubble ${m.role === "user" && editingMessageId === m.id ? "editing" : ""}`}>
                      {m.role === "user" && editingMessageId === m.id ? (
                        <div className="inline-edit-wrap">
                          ...
                          <div className="inline-edit-actions">
                            <button type="button" className="inline-edit-btn ghost" onClick={handleCancelEdit}>
                              取消
                            </button>
                            <button
                              type="button"
                              className="inline-edit-btn primary"
                              onClick={() => void handleSubmitEditedQuestion(m.id)}
                        ...
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            code: ({ className, children, ...props }) => (
                              <MarkdownCode inline={!className?.startsWith("language-")} className={className} {...props}>
                                {children}
                              </MarkdownCode>
                            ),
                          }}
                      ...
                      {m.role === "assistant" && m.thinking && m.thinking.trim() && (
                        <div className="thinking-block">
                          ...
                          <div className="thinking-content">
                            {(thinkingExpandedMap[m.id] || m.thinking.length <= THINKING_PREVIEW_LIMIT)
                              ? m.thinking
                      ...
                      {m.role === "assistant" && m.content.trim() && (
                        <div className="message-actions" role="group" aria-label="回答操作">
                          ...
                          >
                            {copiedMessageId === m.id ? (
                              ...
                              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
                              </svg>
                            )}
                          </button>
                          <button
                          ...
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 9V5a3 3 0 0 0-3-3l-1 5-3 3v9h11a3 3 0 0 0 3-3v-5a2 2 0 0 0-2-2h-5z" />
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 10H4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h3z" />
                            </svg>
                          </button>
                          <button
                            type="button"
                          ...
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.9} d="M10 15v4a3 3 0 0 0 3 3l1-5 3-3V5H6a3 3 0 0 0-3 3v5a2 2 0 0 0 2 2h5z" />
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.9} d="M17 14h3a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-3z" />
                            </svg>
                          </button>
                        </div>
                      )}
                    </div>
                    {m.role === "user" && m.content.trim() && editingMessageId !== m.id && (
                      <div className="message-actions user-message-actions" role="group" aria-label="问题操作">
                        <button
                          type="button"
                          className={`message-action-btn ${copiedMessageId === m.id ? "active" : ""}`}
                          title="复制问题"
      ...
      <div className="panel-footer border-transparent bg-transparent flex flex-col items-center gap-2">
        <div className="w-full max-w-3xl mx-auto mt-1">
          <div className="relative composer-shell">
            ...
            <div className="composer-mode-row">
              ...
              >
                {isGenerating ? (
                  <svg className="send-stop-icon" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <rect x="6" y="6" width="12" height="12" rx="2.4" />
                  </svg>
                ) : (
                  <>
                    <svg className="send-arrow-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 11.5L20 4l-5 16-3.5-6L3 11.5z" />
                    </svg>
                    发送
                  </>
                )}
        ...
        <div className="text-[10px] text-(--muted) text-center">内容由 AI 生成，请注意甄别。</div>
      </div>

      {configProvider && (
        <div className="modal-backdrop z-80" onMouseDown={() => closeProviderConfig()}>
          <div className="modal-card w-[min(540px,94vw)] p-8" onMouseDown={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0 flex-1">
                <h3 className="modal-title text-left">配置 {configProvider.label}</h3>
            ...
            <div className="mt-6 flex flex-col gap-4">
              ...
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" className="btn btn-ghost" onClick={() => closeProviderConfig()} disabled={configSaving}>
                  取消
                </button>
                <button type="button" className="btn btn-primary" onClick={() => void handleSaveProviderConfig()} disabled={configSaving}>
