...
test("completes the forgot password flow on desktop and mobile", async ({
  page,
}) => {
  const runtimeErrors = collectRuntimeErrors(page);
  await prepareLoggedOutPage(page);
  await page.route(
    "http://127.0.0.1:8000/system-auth/password-reset/send-code",
    (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    "http://127.0.0.1:8000/system-auth/password-reset/confirm",
    (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.goto("/");
  await page.getByPlaceholder("输入邮箱地址").fill("reader@example.com");
  ...
  await expect(page.getByText("验证码: 123456")).toBeVisible();
  await page.getByPlaceholder("验证码").fill("123456");
  await page.getByPlaceholder("设置新密码").fill("new secure password");
  await page.getByPlaceholder("确认新密码").fill("new secure password");

  const screenshotDir = process.env.FORGOT_PASSWORD_SCREENSHOT_DIR;
  if (screenshotDir) {
    await page.screenshot({
      path: `${screenshotDir}/forgot-password-desktop.png`,
    });
  }

  await page.getByRole("button", { name: "确认重置" }).click();
  await expect(page.getByRole("heading", { name: "输入密码" })).toBeVisible();
  ...
  expect(mobileLayout.cardBottom).toBeLessThanOrEqual(
    mobileLayout.viewportHeight,
  );

  if (screenshotDir) {
    await page.screenshot({
      path: `${screenshotDir}/forgot-password-mobile.png`,
    });
  }
