...
- **单条视频导入**：支持粘贴 B 站视频 URL，也支持从电脑或手机选择本地视频文件上传，转写后进入当前知识库。
...
ECS 上的镜像发布、Nginx 接入、精确 SHA 上线与回滚步骤见[容器化生产部署手册](docs/deployment/container-production.md)。
...
1. **登录**：使用系统账号登录；需要批量导入 B 站收藏夹时，再扫码绑定 B 站账号。
2. **选择知识库**：在侧栏选择或创建知识库，导入内容都会进入当前知识库。
3. **导入资料**：可以导入 B 站收藏夹，也可以通过 `导入视频` 粘贴视频 URL 或上传本地视频。
4. **同步与构建**：收藏夹资料可同步收藏夹变更；单条视频导入会创建导入任务并自动转写、入库、写入向量。
5. **使用**：语义检索或 RAG 问答，回答中尽量引用来源视频。手机端可直接提问，按需展开侧栏管理资料或账号。

---

## 导入视频 URL / 本地视频

导入前请先在侧栏选中一个知识库；没有选中知识库时，导入按钮会保持不可用或后端返回请选择知识库。

**导入视频 URL**

1. 点击侧栏的 `+ 导入`，选择 `导入视频`。
2. 保持 `视频 URL` 页签。
3. 粘贴 B 站视频链接，例如 `https://www.bilibili.com/video/BV...`。
4. 点击 `开始导入`，等待任务创建成功。完成后可在当前知识库中检索或提问。

**导入本地视频**

1. 点击侧栏的 `+ 导入`，选择 `导入视频`。
2. 切换到 `本地视频` 页签。
3. 点击文件选择框，从电脑文件或手机相册/文件中选择视频。
4. 点击 `开始导入`。后端会上传视频、调用 ASR 转写、写入当前知识库并建立向量。

本地视频导入依赖 DashScope ASR 与 ffmpeg；如果视频没有可识别语音、ASR Key 未配置、ffmpeg 不可用或网络异常，导入任务可能失败。失败后可以重新选择文件导入。
...
**Q：前端连不上后端？**  
A：确认后端已启动、端口一致；开发环境下后端 CORS 允许本地与局域网来源以及 Capacitor 静态 APK 访问，若仍失败请检查浏览器控制台、接口基地址、局域网 IP 与防火墙。
