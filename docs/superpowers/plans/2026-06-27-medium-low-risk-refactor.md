...
- [x] **Step 5: Clean stale Next dev type cache before build**

After merging to main and synchronizing `node_modules`, `npm run build` failed because a stale `.next/dev/types` file generated by the previous Next.js version entered type checking. Add a `prebuild` cleanup script for `.next/dev/types` and lock it with a structure test.

Run:

- `python -m pytest tests/test_frontend_structure.py -q -k build_cleans_next_dev_type_cache`
