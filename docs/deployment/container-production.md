...
在阿里云容器镜像服务 ACR 的北京地域创建一个私有命名空间，并创建 `zhiku-backend`、`zhiku-frontend` 两个私有仓库。为 GitHub Actions 创建仅能向该命名空间推送镜像的最小权限账号，不要使用阿里云主账号凭据。
...
先安装 Docker Engine、Docker Compose 插件和 Nginx。选择一个固定的部署账号；Docker 登录和运行部署脚本必须使用同一账号，否则脚本读取不到该账号保存的 ACR 凭据。下面示例使用 root shell：

```bash
sudo -i
install -d -m 0755 /opt/zhiku-cloud/{deploy,scripts,data,logs,backups}
cd /opt/zhiku-cloud
```
...
将 `deploy/.env.deploy.example` 复制为 `deploy/.env.deploy`，填写北京 ACR 地址、命名空间和正式域名。另行创建应用运行配置 `deploy/.env.production`。两个环境文件只保存在服务器，不进入 Git，不打进镜像：

```bash
cd /opt/zhiku-cloud
cp deploy/.env.deploy.example deploy/.env.deploy
install -m 0600 /dev/null deploy/.env.production
chmod 600 deploy/.env.deploy deploy/.env.production
chmod 755 scripts/deploy.sh
```

`deploy/.env.production` 应填写正式环境的数据库、管理员邮箱、加密密钥、SMTP、Google 登录等应用变量。生产值和秘密始终留在宿主机；禁止提交到 Git、Dockerfile、Compose 文件或镜像层。

首次登录 ACR，密码通过标准输入提供，避免出现在 shell 历史中：

```bash
read -rsp 'ACR password: ' ACR_PASSWORD && echo
printf '%s' "$ACR_PASSWORD" | docker login registry.cn-beijing.aliyuncs.com --username 'YOUR_ACR_USERNAME' --password-stdin
unset ACR_PASSWORD
```

以后如果部署基础设施有变更，需要再次同步 `compose.production.yml`、`scripts/deploy.sh`、`deploy/nginx/zhiku-cloud.conf.example` 和 `.env.deploy.example` 的结构变化。同步示例文件时不要覆盖服务器上的 `.env.deploy`、`.env.production` 或实际证书路径。
...
仅在确认数据库或 Chroma 数据损坏、且已选择正确的 `backups/<时间-SHA>/data.tar.gz` 后执行。先停后端，把归档解压到单独的临时目录检查，不要直接覆盖生产目录：

```bash
cd /opt/zhiku-cloud
export IMAGE_TAG="$(tr -d '[:space:]' < deploy/current-version)"
docker compose --project-name zhiku-cloud --env-file deploy/.env.deploy -f compose.production.yml stop backend

RESTORE_DIR=$(mktemp -d /opt/zhiku-cloud/restore.XXXXXX)
tar -xzf /opt/zhiku-cloud/backups/REPLACE_ME/data.tar.gz -C "$RESTORE_DIR"
find "$RESTORE_DIR" -maxdepth 3 -ls
```

检查 SQLite 与 Chroma 内容属于同一次备份。然后把现有数据目录改名为带时间戳的安全副本，再明确创建并填充新的数据目录；以下属高风险人工操作，路径必须逐项核实：

```bash
SAFETY_COPY="/opt/zhiku-cloud/data.safety.$(date -u +%Y%m%dT%H%M%SZ)"
mv /opt/zhiku-cloud/data "$SAFETY_COPY"
install -d -m 0750 /opt/zhiku-cloud/data
cp -a "$RESTORE_DIR"/. /opt/zhiku-cloud/data/
chown -R --reference="$SAFETY_COPY" /opt/zhiku-cloud/data

docker compose --project-name zhiku-cloud --env-file deploy/.env.deploy -f compose.production.yml up -d --pull never backend
curl --fail http://127.0.0.1:8000/health
curl --fail https://zhiku-cloud.cn/health
```

确认服务与数据正常后，再按维护策略清理临时目录和旧副本。整个流程不删除 Compose volume，也不批量清理 Docker 数据。
