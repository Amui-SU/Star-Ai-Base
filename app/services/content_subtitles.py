...
async def _download_selected_subtitle_with_timeline(
    bili: Any,
    bvid: str,
    subtitles: list[dict[str, Any]],
    *,
    ...
    Returns:
        (text, timeline) 或 None
    """
    selected_subtitle = pick_preferred_subtitle(subtitles)
    subtitle_url = extract_subtitle_url(selected_subtitle or {})
    if subtitle_url:
        if hasattr(bili, "download_subtitle_with_timeline"):
            subtitle_text, timeline = await bili.download_subtitle_with_timeline(
                subtitle_url
            )
        else:
            subtitle_text = await bili.download_subtitle(subtitle_url)
        ...
        if subtitle_text and len(subtitle_text) >= 50:
            preview = subtitle_text[:120].replace("\n", " ").strip()
            logger.info(
                f"[{bvid}] 字幕获取成功{success_suffix}，长度={len(subtitle_text)}，时间轴={len(timeline)}条，预览：{preview}"
            )
            return subtitle_text, timeline
        logger.info(f"[{bvid}] 字幕内容过少，已忽略")
    else:
        logger.info(f"[{bvid}] 字幕地址为空，无法下载")
    return None
...
async def try_bilibili_subtitle_with_timeline(
    bili: Any,
    bvid: str,
    cid: int,
    *,
    ...
    try:
        ...
        subtitles = extract_subtitles(player_info or {})
        if subtitles:
            result = await _download_selected_subtitle_with_timeline(
                bili, bvid, subtitles
            )
            if result:
                return result
